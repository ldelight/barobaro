...
public class ExportCertPasswordWindowXK extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.crypto_export_cert_password_window);
		...
		mCallMode = getIntent ().getStringExtra (ExportCertPasswordWindowXK.mCallModeKey);
	...
	private void onOKButtonClick (View v)
	{
		...
		aIntent = new Intent(this, ExportCertWithCertShare.class);
		aIntent.putExtra (ExportCertWithCertShare.mMediaIDKey, mMediaID);
		aIntent.putExtra (ExportCertWithCertShare.mSelectedCertDataKey, mSelectedData.getValueArray ());
		
		//aIntent.putExtra (ExportCertWithCertShare.mSelectedCertPasswordKey, m_P_assW_ord);
		aIntent.putExtra (ExportCertWithCertShare.mRandomValueKey, mRandomValue);
		aIntent.putExtra (ExportCertWithCertShare.mE_ncryptedDataKey, mEncryptedData);
		
		aIntent.addFlags (Intent.FLAG_ACTIVITY_FORWARD_RESULT);
		startActivity(aIntent); 
		XUtil.resetByteArray(m_P_assW_ord);
		finish ();
	...
	protected void onActivityResult (int requestCode, int resultCode, Intent data)
	{
		if (resultCode == XKConstants.XKKeypadError.MakeIndexError)
		{
			 ...
			 *--------------------------------------------------------------------------------------*/
			Toast.makeText (ExportCertPasswordWindowXK.this, "인덱스 생성에 실패하였습니다.", Toast.LENGTH_SHORT).show ();
		}
	...
	public void showToast(final String message)
	{
		mHandler.post(new Runnable()
		{
			...
			public void run()
			{
				Toast.makeText(ExportCertPasswordWindowXK.this, message, Toast.LENGTH_LONG).show();
			}
		});
	}	
}
